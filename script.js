...
function fadeIn() {
  ...
  document
    .getElementById("category-tag")
    .removeEventListener("click", switchEffect);
  document
    .getElementById("category-tag")
    .addEventListener("click", FadeIn_switch);
}
function FadeIn_switch() {
  ...
  document
    .getElementById("location-tag")
    .addEventListener("click", switchEffect_switch);
}
...
function loadCat(id) {
  var cat = document.getElementById("cat_area");
  var r = new XMLHttpRequest();

  r.onreadystatechange = function () {
    if (r.readyState == 4) {
      var t = r.responseText;
      cat.classList.remove("justify-content-center");
      cat.classList.remove("align-items-center");
      cat.classList.remove("d-flex");
      cat.classList.add("pt-3");
      document.getElementById("cat_area").innerHTML = t;
    }
  };

  r.open("GET", "catLoadProcess.php?cid=" + id, true);
  r.send();
}
...
function loginSwift() {
  ...
  anime({
    ...
    easing: "easeInOutQuad",
    begin: () => {
      document.querySelector(".signup-back").style.borderRadius =
        " 0 1.3em 1.3em 0";
    },
  });
}
...
function search() {
  var txt = document.getElementById("form-eff");
}

// Define a variable to store the selected city's ID
let selectedCityId = null;

// Get all the city div elements
const cityDivs = document.querySelectorAll(".city");

// Add click event listeners to the city divs
cityDivs.forEach(function (cityDiv) {
  cityDiv.addEventListener("click", function () {
    // Remove the "selected" class from all city divs
    cityDivs.forEach(function (div) {
      div.classList.remove("selected");
    });

    // Add the "selected" class to the clicked city div
    cityDiv.classList.add("selected");

    // Extract the city ID from the div's id attribute
    selectedCityId = parseInt(cityDiv.id.replace("city", ""));
  });
});
